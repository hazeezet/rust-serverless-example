...
```bash
rust-serverless-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rust-serverless-api$ sam local start-api
rust-serverless-api$ curl http://localhost:3000/
```
...
```bash
rust-serverless-api$ sam logs -n HelloWorldFunction --stack-name rust-serverless-api --tail
```
